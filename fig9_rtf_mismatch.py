...
def main():
    print("--- GENERATING FIG 9: RTF MISMATCH SWEEP (FINAL NOMENCLATURE) ---")
    fs, n_fft, hop_length, M = 16000, 1024, 256, 4
    
    # Mismatch from 0 to 20 degrees
    mismatches = np.linspace(0, 20, 10)
    ...
    for mm in mismatches:
        print(f">>> Simulating Mismatch: {mm:.1f} deg <<<")
        ...
        for seed_val in range(42, 42 + num_seeds):
            ...
            # Static geometry: Target at 90, Interferer at 45
            mix, target, _, _ = simulator.simulate(n=1, reverb=True, target_rt60=0.2, interferer_angles=[45], save_outputs=False)
            ...
            d, c = 0.05, 343.0
            mm_rad = np.radians(mm)
            # Phase shift corresponding to angular mismatch
            phase_shifts = np.exp(1j * 2 * np.pi * freqs[np.newaxis, :] * (np.arange(M)[:, np.newaxis] * d / c) * np.sin(mm_rad))
            
            # Transpose phase_shifts to (513, 4) to match oracle_rtf
            assumed_rtf = oracle_rtf * phase_shifts.T
            ...
            # Subspace projection error calculated using the ASSUMED (mismatched) RTF
            d_exp = assumed_rtf[:, np.newaxis, :]
            num = np.abs(np.sum(d_exp.conj() * u1, axis=-1))
            d_norm = np.linalg.norm(assumed_rtf, axis=-1)[:, np.newaxis]
            ...
            w_nom = ulb.process(R_matrix, assumed_rtf, mu_inv, zeta=z_nom)
            ...
            w_hev = ulb.process(R_matrix, assumed_rtf, mu_inv, zeta=z_hev)
            ...
            # Make sure we use assumed_rtf here as well
            z_wng = get_wng_constrained_loading(R_matrix, assumed_rtf, gamma_db=0.0, z_init_max=0.5)
            w_wng = ulb.process(R_matrix, assumed_rtf, mu_inv, zeta=z_wng * trace_power)
            ...
            w_hard = ulb.process(R_matrix, assumed_rtf, mu_inv, zeta=z_hard)
            ...
            w_cont = ulb.process(R_matrix, assumed_rtf, mu_inv, zeta=z_cont)
    ...
    for m_key, m_info in models.items():
        lw = m_info.get('lw', 2)
        axs[0].plot(mismatches, results[m_key]['sisdr'], color=m_info['color'], linestyle=m_info['ls'], linewidth=lw, label=m_info['label'])
        axs[1].plot(mismatches, results[m_key]['wng'], color=m_info['color'], linestyle=m_info['ls'], linewidth=lw, label=m_info['label'])

    axs[0].set_title('(a) Signal Separation Under Mismatch', fontweight='bold')
    axs[0].set_ylabel('SI-SDRi (dB)')
    axs[0].grid(True, linestyle='--', alpha=0.6)
    
    axs[1].set_title('(b) Array Robustness Under Mismatch', fontweight='bold')
    axs[1].set_ylabel('Mean WNG (dB)')
    axs[1].set_xlabel(r'Assumed RTF Angular Mismatch $\Delta\theta_{RTF}$ (°)', fontweight='bold')
    axs[1].grid(True, linestyle='--', alpha=0.6)
    
    # UPDATED LEGEND: ncol=5 and columnspacing=1.0 to fit cleanly in one row
    ...
    plt.savefig('results/fig9_rtf_mismatch.pdf', dpi=300, bbox_inches='tight') 
    print("\n✅ Saved 'results/fig9_rtf_mismatch.pdf'")
