...
DEFAULT_RT60 = 0.5   
SNR_TARGET_DB = 60  # Set to 60dB (imperceptible) to act purely as mathematical matrix regularization
SIR_TARGET_DB = 0   
...
def get_audio_files(dataset_name, n_needed, min_duration=5.0):
    ...
    try:
        ...
        for f in files:
            if len(valid_files) >= n_needed: break
            try:
                info = sf.info(f)
...
def add_awgn(signal, snr_db):
    ...
    if sig_power == 0: return signal, np.zeros(signal.shape)
...
def simulate_scene(n=1, reverb=True, target_rt60=0.5, interferer_angles=None, save_outputs=False):
    ...
    target_files = get_single_dataset_files('ljspeech', 1)
    
    # Interferers are drawn from LibriSpeech (and MUSAN if >1 interferer)
    interferer_files = []
    if n > 0:
        interferer_files += get_single_dataset_files('librispeech', max(1, n - 1))
    if n > 1:
        interferer_files += get_single_dataset_files('musan_speech', 1)
    ...
    # --- 2. Setup Room ---
    if reverb:
        e_absorption, max_order = pra.inverse_sabine(target_rt60, ROOM_DIM)
        # FIX: We use the exact 'max_order' calculated by the inverse_sabine formula.
        # Previously, hardcoding max_order=15 prematurely truncated the ray-tracing 
        # algorithm, physically preventing the room from reaching the requested RT60.
    ...
    mic_center = np.mean(MIC_LOCS, axis=1) 
    RADIUS = 2.0 
    MIN_SRC_DIST = 0.5 
    placed_sources = []
    ...
    pos_target = [
        mic_center[0] + RADIUS * np.cos(target_angle),
        mic_center[1] + RADIUS * np.sin(target_angle),
        1.5
    ]
    ...
    if n > 0:
        for i in range(n):
            ...
            for attempt in range(max_attempts):
                ...
                cand_x = mic_center[0] + RADIUS * np.cos(theta)
                cand_y = mic_center[1] + RADIUS * np.sin(theta)
                candidate = np.array([cand_x, cand_y, 1.5])
                
                collision = any(np.linalg.norm(candidate - es) < MIN_SRC_DIST for es in placed_sources)
                if not collision:
    ...
    if n > 0:
        for i, i_sig in enumerate(interferer_sigs):
            ...
            interf_ch1_total += i_ch1[:final_len]
            interf_ch2_total += i_ch2[:final_len]
    ...
    if p_interf > 0:
        scaling_factor = np.sqrt(p_target / (p_interf * (10 ** (SIR_TARGET_DB / 10))))
